...
func NewMqMsgStream(ctx context.Context,
	...
	ctxLog := log.Ctx(ctx)
	stream.enableProduce.Store(!paramtable.Get().CommonCfg.IsBackupInstance.GetAsBool())
	paramtable.Get().Watch(paramtable.Get().CommonCfg.IsBackupInstance.Key, config.NewHandler("is backup instance setting", func(event *config.Event) {
...
func (ms *mqMsgStream) Produce(msgPack *MsgPack) error {
	if !ms.isEnabledProduce() {
		...
		return merr.ErrDenyProduceInBackup
	}
...
func (ms *mqMsgStream) Broadcast(msgPack *MsgPack) (map[string][]MessageID, error) {
	...
	if !ms.isEnabledProduce() && !isCreateCollectionMsg {
		...
		return ids, merr.ErrDenyProduceInBackup
	}
